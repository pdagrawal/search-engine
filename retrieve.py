...
def get_postings_for_query(query_words):
    ...
    for word in query_words:
        ...
        for i in range(0,int(dictionaries[word_index+1])):
            ...
            postings_list[word][posting.split(',')[0]] = posting.split(',')[1]
    return postings_list

# Read postings file to get term weights
def get_postings_for_word(word):
    postings_list = {}
    with open('output_files/postings_file.txt', 'r') as postings:
        for line in postings:
            if line.split()[0] == word:
                for doc in line.split()[1:]:
                    postings_list[doc] = float(doc.split(':')[1])
                break
    return postings_list
